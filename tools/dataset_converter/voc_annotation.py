...
parser.add_argument('--dataset_path', type=str, help='path to PascalVOC dataset, default=%(default)s', default=os.getcwd()+'/../final_yolo/scaphoid')
...
for year, image_set in sets:
    ...
    for image_id in image_ids:
        file_string = '%s/VOC%s/JPEGImages/%s.bmp'%(dataset_realpath, year, image_id)
        # check if the image file exists
        if not os.path.exists(file_string):
            file_string = 'D:/final_yolo/scaphoid/VOC2007/JPEGImages/%s.bmp'%(image_id)
        if not os.path.exists(file_string):
